...
export const QuotationProcess: React.FC = () => {
  ...
  return (
    <section className="py-12 sm:py-16 bg-[#0B0F19] border-y border-slate-800/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-2xl mx-auto mb-10 sm:mb-14">
          <p className="text-[11px] uppercase tracking-luxury text-amber-400 font-semibold mb-2">
            Clear & Direct
          </p>
          <h2 className="font-serif text-2xl sm:text-3xl lg:text-4xl text-white font-normal tracking-tight">
            How Our Quotation Works
          </h2>
          <p className="text-xs sm:text-sm text-slate-200 mt-2 max-w-md mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 relative">
          {steps.map((step, index) => (
            <div
              key={index}
              className="relative bg-[#111827] rounded-xl p-6 sm:p-7 border border-slate-800 hover:border-amber-500/40 transition-all shadow-xl flex flex-col justify-between"
            >
              <div>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="font-serif text-2xl font-bold text-amber-500/50">
                    {step.number}
                  </span>
                </div>

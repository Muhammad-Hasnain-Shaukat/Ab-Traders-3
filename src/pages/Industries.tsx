...
export const Industries: React.FC = () => {
  return (
    <div className="pt-24 pb-16 sm:pt-28 sm:pb-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="mb-12 bg-slate-900 border border-slate-800 rounded-xl p-4 sm:p-5 flex items-start gap-3.5 max-w-3xl mx-auto shadow-inner">
      ...
      <div className="space-y-12">
        {INDUSTRIES.map((industry, index) => {
          ...
          return (
            <div
              key={industry.id}
              className="bg-[#111827] border border-slate-800 rounded-2xl overflow-hidden shadow-2xl grid grid-cols-1 lg:grid-cols-12 items-center"
            >
              {/* Photo */}
              <div
                className={`lg:col-span-5 h-64 lg:h-full min-h-[280px] bg-slate-900 relative overflow-hidden ${
              ...
              <div className="lg:col-span-7 p-6 sm:p-10 lg:p-12 space-y-4">
                ...
                {matchingProducts.length > 0 && (
                  <div className="pt-2">
                    <p className="text-xs font-semibold text-slate-300 mb-2">Recommended Packaging:</p>
                    <div className="flex flex-wrap gap-2">
                      {matchingProducts.slice(0, 4).map((p) => (
                        <Link
                          key={p.id}
                          to={`/product/${p.slug}`}
                          className="text-xs bg-slate-900 hover:bg-slate-800 border border-slate-800 hover:border-amber-500/40 px-3 py-1.5 rounded text-slate-300 transition-colors truncate max-w-xs"
                        >
                          {p.name}
                        </Link>
                      ))}
                ...
                <div className="pt-4">
                  <Link
                    ...
                    className="inline-flex items-center gap-2 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 text-xs sm:text-sm font-bold px-5 py-2.5 rounded-md transition-all shadow-lg shadow-amber-500/20"
                  >
